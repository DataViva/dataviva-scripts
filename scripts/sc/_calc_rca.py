...
def calc_rca(ybc, year):
    ...
    for geo_level in [2, 4, 8]:
        ...
        course_criterion = ybc_data["course_id"].map(lambda x: len(x) == 5)
        ybc_data = ybc_data[bra_criterion & course_criterion]
        
        ybc_data = ybc_data[["bra_id", "course_id", "students"]]
        
        ybc_data = ybc_data.pivot(index="bra_id", columns="course_id", values="students")
        ybc_data_rca = growth.rca(ybc_data)
