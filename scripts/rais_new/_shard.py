...
def shard(ybio, raw):
    ...
    #
    #         for i, (col_name, d) in enumerate(zip(my_nesting_cols, depths)):
    #             s = ','.join([str(z) for z in depths[:i+1]])
    #             if s not in seen:
    #                 print "i:",i, s, col_name, "depth:",d
    #                 if i > 0:
    #                     my_raw = use[i-1]
    #                 else:
    #                     my_raw = raw.copy()
    ...
    #                 my_raw[col_name] = my_raw[col_name].str[:d]
    #                 print my_raw.head()
    #                 raw_input('')
